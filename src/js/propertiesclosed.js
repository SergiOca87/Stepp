...
var Properties = new Vue({
	...
	watch: {
		closed_properties: function() {
			...
			for (var i = 0; i < this.closed_properties.length; i++) {
				var property = this.closed_properties[i];
	...
	methods: {
		...
		fetchProperties: function() {
			...
			return jQuery.getJSON(bootstrap.resource_path.properties, function(
				...
				// populate properties	
				vm.properties = vm.closed_properties = response;
				vm.properties.sort(this.sortPropertiesByPrice)
				vm.loading = false;
			});
		},
		...
		search: function() {
			...
			if (!active_params.length && include_map_bounds === false) {
				this.closed_properties = this.properties;
				return;
			} else {
				// clear filtered properties
				this.closed_properties = [];
			}
			...
			for (var i = 0; i < this.properties.length; i++) {
				...
				if (pass === true && this.properties[i].status !== 'active' ) {
					this.closed_properties.push(this.properties[i]);
				}
			}
		...
		paginate: function() {
			...
			Object.keys(this.closed_properties).forEach(key => {
				vm.paginated_properties.push(vm.closed_properties[key]);
			});

			this.paginated_properties.sort(this.sortPropertiesByPrice).splice(
				this.per_page * this.page,
				this.closed_properties.length - this.per_page * this.page
			);
		},
